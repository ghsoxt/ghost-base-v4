...
RegisterCommand('remcar',function(source,args,rawCommand)
    ...
    if vRP.hasPermission(user_id,"mindmaster.permissao") then
        if args[1] and args[2] then
            ...
            local identity = vRP.getUserIdentity(user_id)
            local identitynu = vRP.getUserIdentity(nuser_id)
            vRP.execute("nation_conce/removeUserVehicle",{ user_id = parseInt(args[2]), vehicle = args[1], ipva = parseInt(os.time())  }) 
...
RegisterCommand('reset',function(source,args,rawCommand)
    ...
    local identity = vRP.getUserIdentity(user_id)
    if user_id then
        if vRP.hasPermission(user_id,"mindmaster.permissao") or vRP.hasPermission(user_id,"diretor.permissao") or vRP.hasPermission(user_id,"administrador.permissao") then
            if args[1] then
                local nplayer = vRP.getUserSource(parseInt(args[1]))
...
RegisterCommand('status',function(source,args,rawCommand)
    ...
    if vRP.hasPermission(user_id,"ac.permissao") then
        ...
        local cartel = vRP.getUsersByPermission("carteldemedelin.permissao")
        local yakuza = vRP.getUsersByPermission("yakuza.permissao")
        local triade = vRP.getUsersByPermission("triade.permissao")
        local grota = vRP.getUsersByPermission("grota.permissao")
        local turquia = vRP.getUsersByPermission("turquia.permissao")
        local franca = vRP.getUsersByPermission("franca.permissao")
        local canada = vRP.getUsersByPermission("canada.permissao")
        local colombia = vRP.getUsersByPermission("colombia.permissao")
        local egito = vRP.getUsersByPermission("egito.permissao")
        local camorra = vRP.getUsersByPermission("camorra.permissao")
        local tequila= vRP.getUsersByPermission("tequila.permissao")
        ...
        local user_id = vRP.getUserId(source)        
        TriggerClientEvent("Notify",source,
        "importante","<bold><b>Jogadores</b>: <b>"..onlinePlayers..
        "<br>Administração</b>: <b>"..#staff..
        "<br>Staff em RP: <b>"..#staffoff..
        "</b><hr>Policiais</b>: <b>"..#policia..
        "<br>Paramédicos</b>: <b>"..#paramedico..
        "<br>Mecânicos</b>: <b>"..#mec..
        "<br>Membros de Faccao</b>: <b>"..#ilegal..
        "<hr>Bratva (Arma)</b>: <b>"..#bratva..
        "<br>Cartel (Arma)</b>: <b>"..#cartel..
        "<br>Yakuza (Munição)</b>: <b>"..#yakuza..
        "<hr>Triade (Munição)</b>: <b>"..#triade..
        "<br>Grota (Maconha)</b>: <b>"..#grota..
        "<br>Turquia (Maconha)</b>: <b>"..#turquia..
        "<hr>França (Cocaina)</b>: <b>"..#franca..
        "<br>Canada (Lsd)</b>: <b>"..#canada..
        "<br>Colombia (Cocaina)</b>: <b>"..#colombia..
        "<hr>Egito (Lsd)</b>: <b>"..#egito..
        "<br>Camorra (Contrabando)</b>: <b>"..#camorra..
        "<br>Tequila (Contrabando)</b>: <b>"..#tequila..
        "<hr>Motoclub (Desmanche)</b>: <b>"..#motoclub..
        "<br>Hells (Desmanche)</b>: <b>"..#hells..
        "<br>Vanilla (Lavagem)</b>: <b>"..#vanilla..
        "<br>Bahamas (Lavagem)</b>: <b>"..#bahamas.."</bold>",9000)
    end
end)
