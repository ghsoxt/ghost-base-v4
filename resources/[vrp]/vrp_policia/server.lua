...
--[ WEBHOOK ]------------------------------------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local webhookocorrencias = "---------------------1082544650818162759/wJ7s-hdS7GoMLJTGWfJpqFmFc8rzijGCePQ-uzVD7Y88pUI_fRpBIbyq94bryfCC_oIU"
local webhookprender = "---------------------1082544569348001852/6Suk4U24LzwhZi_5vZ-BPJHMDIonvCrK_IyAPuyIjk8-4b0iAQ88PW40VEDh9XKoupS9"
local webhookparamedico = "---------------------1082545728288739398/uhAOj-H-NHnEK-Sssaq4V82FoqIEYoMkY43HEyw5nUMHd3Zll5_z1HNkr2i_9OvE_XSp"
local webhookpolicia = "---------------------1082544443875409991/M92GXNhU5QaQmvmWJDgNBYeyxMIZOvCYUhAQTBm_7vcfbVtRP4hC8JbbIqLh4k1BG3ar"
local webhookarsenal = "---------------------1082540917480771686/ccrwu4Jyn3K2aZFms9kXXZKXyLUIDiDkocCPhbHLGRLHuxTNPdL2-Au52qWmdYUvRM2Z"
local webhookmecanico = "---------------------1082546037979353137/LxwoSdy3-vT9_lFQZtJgTHKZLulTC83_FWqnHyglVwchDnCKFMwMJMAHINFQ2L7RdYzx"
local webhookmultas = "---------------------1082544520702472203/bfF0U0le1NyheUsHrIjx4qbC56Sr_tcr90jtQMOn7KLbR9B6quHezWcENsJckB8X3DEJ"
local webhookdetido = "---------------------1082544865105158235/Cm_5N3kUYt3hRYvUKaLPXZgFz2rhUKTxVYjlT-7Yi5LLRrUxRHj9IWpPWzs2PzGTbTgd"
local webhookpoliciaapreendidos = "---------------------1082544746955800596/VvGWXZO0fUiQ4Q8ARTyvgWPKd63ODWlMPyMwOqITLX6Wa_2lXIpp_d_Vv4_QiD1GtpmG"
local webhookArmaAdmin = "---------------------1082545252415574087/p1ybYNksNDBqKdFmTx7ZcW3vMOVNdEKu1aHFiheo0fe8dOLFwj1hjwPwMUjNLzGG6w7Y"
local webhookToogleAdm = "---------------------1082545157238440047/5IxL5iK67lIPzCADac-ysuT-oE7vOfyNLKBZaBbiDIrNDQv2mW8N8VhJ4pfl5E94rFjp"
local webhookAdv = "---------------------1082544992570056715/jLQyoa1UivfguGAGzpqkpRTq2ddwYzGeHLsOxeWEt10Kj_ZpzZ2CJihMctK7j4DKlkFJ"
...
RegisterCommand('placa',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"ajuda.permissao") or vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"paramedico.permissao") or vRP.hasPermission(user_id,"mecanico.permissao") or vRP.hasPermission(user_id,"municao.permissao") then
		if args[1] then
...
RegisterCommand('ems2', function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"paramedico.permissao") or vRP.hasPermission(user_id,"moderador.permissao") then
		for k,v in ipairs(oficiais) do
			local identity = vRP.getUserIdentity(parseInt(v))
...
RegisterCommand('ptr2', function(source,args,rawCommand)
	...
	local paramedicos = 0
	local oficiais_nomes = ""
	if vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"moderador.permissao") then
		for k,v in ipairs(oficiais) do
			local identity = vRP.getUserIdentity(parseInt(v))
			oficiais_nomes = oficiais_nomes .. "<b>" .. v .. "</b>: " .. identity.name .. " " .. identity.firstname .. "<br>"
			paramedicos = paramedicos + 1
...
RegisterCommand('mec2', function(source,args,rawCommand)
 	...
 	local paramedicos = 0
 	local oficiais_nomes = ""
 	if vRP.hasPermission(user_id,"mecanico.permissao") or vRP.hasPermission(user_id,"moderador.permissao") then
 		for k,v in ipairs(oficiais) do
 			local identity = vRP.getUserIdentity(parseInt(v))
 			oficiais_nomes = oficiais_nomes .. "<b>" .. v .. "</b>: " .. identity.name .. " " .. identity.firstname .. "<br>"
 			paramedicos = paramedicos + 1
...
RegisterCommand('multar',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"paramedico.permissao") or vRP.hasPermission(user_id,"administrador.permissao") or vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"mecanico.permissao") then
...
RegisterCommand('staffoff', function(source,args,rawCommand)
	...
	local paramedicos = 0
	local oficiais_nomes = ""
	if vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"administrador.permissao") or vRP.hasPermission(user_id,"ac.permissao") then
		for k,v in ipairs(oficiais) do
			local identity = vRP.getUserIdentity(parseInt(v))
			oficiais_nomes = oficiais_nomes .. "<b>" .. v .. "</b>: " .. identity.name .. " " .. identity.firstname .. "<br>"
			paramedicos = paramedicos + 1
...
RegisterCommand('toogle',function(source,args,rawCommand)
	...
	-------------------------------------------------------------------------------------------------------------------------------------
		...
		TriggerClientEvent("Notify",source,"aviso","Você saiu de serviço.")
	elseif vRP.hasPermission(user_id,"paisanasubcomando.permissao") then
	...
	elseif vRP.hasPermission(user_id,"paisanasubtenente.permissao") then
		...
		TriggerClientEvent("Notify",source,"sucesso","Você entrou em serviço.")
	-------------------------------------------------------------------------------------------------------------------------------------
elseif vRP.hasPermission(user_id,"aspirante.permissao") then
...
elseif vRP.hasPermission(user_id,"paisanaaspirante.permissao") then
	...
	TriggerClientEvent("Notify",source,"sucesso","Você entrou em serviço.")
-------------------------------------------------------------------------------------------------------------------------------------
elseif vRP.hasPermission(user_id,"comandantegeral.permissao") then
	vRP.addUserGroup(user_id,"PaisanaComandanteGeral")
	TriggerClientEvent("Notify",source,"aviso","Você saiu de serviço.")
elseif vRP.hasPermission(user_id,"paisanacomandantegeral.permissao") then
...
RegisterCommand('toogleadm',function(source,args,rawCommand)
	...
	elseif vRP.hasPermission(user_id,"administrador.permissao") then
		...
		vRP.removeUserGroup(user_id,"administrador157")
		TriggerClientEvent("Notify",source,"aviso","Você saiu de serviço.")
		SendWebhookMessage(webhookToogleAdm,"```prolog\n[ADM]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[==========SAIU DE SERVICO=========] "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")	
	elseif vRP.hasPermission(user_id,"paisana-admin.permissao") then
		vRP.addUserGroup(user_id,"administrador157")
...
--[ DETIDO ]-------------------------------------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
vRP._prepare("vehicles/set_detido","UPDATE vrp_user_vehicles SET detido = 1, time = @time WHERE user_id AND vehicle = @vehicle")
vRP.prepare("vehicles/select_Vehicles","SELECT * FROM vrp_user_vehicles WHERE user_id = @user_id AND vehicle = @vehicle")
RegisterCommand('detido',function(source,args,rawCommand)
    ...
    if vRP.hasPermission(user_id,"policia.permissao") then
        ...
        if vehicle then
            ...
            local rows = vRP.query("vehicles/select_Vehicles",{ user_id = parseInt(puser_id), vehicle = vname })
            if rows[1] then
                if parseInt(rows[1].detido) == 1 then
                    ...
                    vRP.execute("vehicles/set_detido",{ user_id = parseInt(puser_id), vehicle = vname, detido = 1, time = parseInt(os.time()) })
...
-- 	local user_id = vRP.getUserId(source)
-- 	if vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"policia.permissao")  or vRP.hasPermission(user_id,"paramedico.permissao") then
-- 		if args[1] then
-- 			local nplayer = vRP.getUserSource(parseInt(args[1]))
-- 			if nplayer == nil then
	...
	if nplayer then
		if not vRPclient.isHandcuffed(source) then
			...
			if vRP.getInventoryItemAmount(user_id,"algema") >= 1 then
				...
				if vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"juiz.permissao") then
					if vRPclient.isHandcuffed(nplayer) then
						TriggerClientEvent('carregar',nplayer,source)
						vRPclient._playAnim(source,false,{{"mp_arresting","a_uncuff"}},false)
						SetTimeout(5000,function()
...
AddEventHandler("vrp_policia:carregar",function()
	...
	local user_id = vRP.getUserId(source)
	local nplayer = vRPclient.getNearestPlayer(source,2)
	if vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"paramedico.permissao") or vRP.hasPermission(user_id,"juiz.permissao") then	
		if nplayer then
			if not vRPclient.isHandcuffed(source) then
				TriggerClientEvent('carregar',nplayer,source)
			end
...
RegisterCommand('rmascara',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"moderador.permissao") or vRP.hasPermission(user_id,"paramedico.permissao")  then
		local nplayer = vRPclient.getNearestPlayer(source,2)
		if nplayer then
...
RegisterCommand('cv',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"administrador.permissao") or vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"paramedico.permissao") then
