...
--------[ COMANDO /FPS ON & OFF ]---------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------
local timecycle = 'default'
RegisterNetEvent('fps:timecycle')
AddEventHandler('fps:timecycle', function()
    SetTimecycleModifier(timecycle)
end)
RegisterCommand("fps",function(source,args,rawCommand)
    if args[1] == "on" then
        timecycle = "downtown_FIB_cascades_opt"
        ...
        TriggerEvent("Notify","sucesso","Otimização Habilitada")
    elseif args[1] == "hard" then
        ...
        TriggerEvent("Notify","sucesso","Otimização HARD Habilitada")
    elseif args[1] == "off" then
        ...
        TriggerEvent("Notify","sucesso","Otimização Desabilitada")
    end
end)
...
Citizen.CreateThread(function()
	while true do
        local players = srcserver.CheckPlayers()
        SetDiscordAppId(1039638825581953124)
        SetDiscordRichPresenceAssetText('')
		SetDiscordRichPresenceAsset("alien")
        SetRichPresence("Jogadores Conectados: "..players)
        SetDiscordRichPresenceAction(0, "JOGAR", "fivem://connect/cfx.re/join")
        SetDiscordRichPresenceAction(1, "DISCORD", "https://discord.gg/9XpCESQXMy")
...
-- -----------------------------------------------------------------------------------------------------------------------------------------
-- --[ TODOS CARROS COM PLACAS DO MERCOSUL ]------------------------------------------------------------------------------------------------
-- -----------------------------------------------------------------------------------------------------------------------------------------
imageUrl = "https://cdn.discordapp.com/attachments/1093230565118967868/1094047430825222195/mercosul.png" -- 
...
local object = CreateDui('https://cdn.discordapp.com/attachments/1093230565118967868/1094047443533975672/Q3uw6V7.png', 540, 300) 
local handle = GetDuiHandle(object)
