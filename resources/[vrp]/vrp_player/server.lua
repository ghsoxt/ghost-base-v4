...
function src.checkRoupas()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.getInventoryItemAmount(user_id,"roupas") >= 1 or vRP.hasPermission(user_id,"diamond.permissao") or vRP.hasPermission(user_id,"Alien.permissao") or vRP.hasPermission(user_id,"platina.permissao") then
			return true 
		else
			TriggerClientEvent("Notify",source,"negado","Você não possui <b>Roupas Secundárias</b> na mochila ou VIP Platina.") 
			return false
		end
	end
end
...
local salarios = {
	...
	{ ['permissao'] = "platina.permissao", ['nome'] = "Platina", ['payment'] = 8000 },
	{ ['permissao'] = "Alien.permissao", ['nome'] = "Alien", ['payment'] = 12000 },
	{ ['permissao'] = "diamond.permissao", ['nome'] = "Diamond", ['payment'] = 20000 },
	...
	{ ['permissao'] = "advogado.permissao", ['nome'] = "Advogado(a)", ['payment'] = 7000 },
	{ ['permissao'] = "desembargador.permissao", ['nome'] = "Desembargador", ['payment'] = 8500 },
	{ ['permissao'] = "promotor.permissao", ['nome'] = "Promotor(a)", ['payment'] = 8500 },
	{ ['permissao'] = "juiz.permissao", ['nome'] = "Juiz", ['payment'] = 10000 },
	...
	{ ['permissao'] = "mecanico.permissao", ['nome'] = "Mêcanico", ['payment'] = 8000 },
}
...
function src.checkMasterPerm()
    ...
    local user_id = vRP.getUserId(source)
    if user_id then
        if vRP.hasPermission(user_id,"Alien.permissao") or vRP.hasPermission(user_id,"ac.permissao") or vRP.hasPermission(user_id,"diamond.permissao") then
            return true 
        else
            TriggerClientEvent("Notify",source,"negado","Você não possui VIP Alien.") 
        end
    end
end
...
RegisterCommand('semems',function(source,args,rawCommand)
    ...
    if vRPclient.getHealth(source) <= 101 then
       if parseInt(#paramedico) >= 1 then 
            ...
            if vRP.request(source, "Você deseja rezar pra ser renascido ? Esse serviço custará R$10000", 60) then
                    if vRP.tryFullPayment(user_id,10000) then
...
RegisterCommand('croupa',function(source,args,rawCommand)
    ...
    if vRP.hasPermission(user_id,"mindmaster.permissao") or vRP.hasPermission(user_id, "Alien.permissao") then
...
-- end)
 ...
 RegisterCommand('cargb',function(source,args,rawCommand)
     local user_id = vRP.getUserId(source)
...
AddEventHandler('survavel:playerdeath',function(nSource,weapon)
			...
			---sendLog('LogMorte',"[ID]:("..user_id..") \n[ASSASINO]: ("..nuser_id..") \n[ARMA]: "..weapon.."\n[LOC ASSASINO]: "..tD(x)..","..tD(y)..","..tD(z).."\n[LOC VITIMA]: "..tD(x2)..","..tD(y2)..","..tD(z2).." ```",true)
			local admin = vRP.getUsersByPermission("ac.permissao")
			for l,w in pairs(admin) do
...
RegisterCommand("vipinicial",function(source,args,rawCommand)
    ...
    if resgate[1].vipteste == 0 then
        ...
        local string = 'vrp.removeGroup("'..user_id..'","Bronze")'
