...
local cfg_itens = module("alien_inventario", "itens")
local garage = Proxy.getInterface("nation_garages")

vRPN = {}
Tunnel.bindInterface("alien_inventario",vRPN)
Proxy.addInterface("alien_inventario",vRPN)

vRPNclient = Tunnel.getInterface("alien_inventario")
...
function vRPN.trunkChestClose()
	...
	if user_id then
		...
		if vehicle then
			vGARAGE.vehiclnyo_moduleseClientTrunk(-1,vnetid,true)
		end
	end
	return false
end
	...
	if nuser_id then
		...
		if user_id then
			if config.nyoGarages then
				...
				local trunk = exports['nyo_modules']:getVehicleTrunk(vname)
				if vehicle then
					if lock == 1 then
						if banned then
							return
		...
		if vida >= 1 then
			if vRP.request(nplayer,"Você está sendo roubado, deseja passar tudo?",30) then
				...
				if vRP.tryPayment(nuser_id,nmoney) then
					...
					TriggerClientEvent("Notify",source,"aviso","Você roubou R$ "..nmoney..".")
...
RegisterCommand('saquear',function(source,args,rawCommand)
	...
	if nuser_id then
		...
		if vida < 1 then
			...
			if vRP.tryPayment(nuser_id,nmoney) then
				vRP.giveMoney(user_id,nmoney)
				TriggerClientEvent("Notify",source,"aviso","Você pegou R$ "..nmoney..".")
			end
