...
klaus = {}
Tunnel.bindInterface("alien_tempban",klaus)
...
vRP.prepare("klaus/getTempban", "SELECT * FROM alien_tempban WHERE user_id = @user_id")
vRP.prepare("klaus/addTempban"," INSERT IGNORE INTO alien_tempban(user_id,tempban) VALUES(@user_id,@tempban)")
vRP.prepare("klaus/remTempban"," DELETE FROM alien_tempban WHERE user_id = @user_id")
vRP.prepare("klaus/setTempban","UPDATE alien_tempban SET tempban = @tempban WHERE user_id = @user_id")
...
function klaus.tempban(id, tempo)
	...
	if vRP.hasPermission(nuser_id,"mindmaster.permissao") then
