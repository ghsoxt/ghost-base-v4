...
vSERVER = Tunnel.getInterface("alien-vendinha")
...
RegisterNUICallback("ButtonClick",function(data,cb)
	...
	if data == "vender-cm" then
		TriggerServerEvent("alien-vendinha:weaponfactory","cm")
		
	elseif data == "vender-rl" then
		TriggerServerEvent("alien-vendinha:weaponfactory","rl")
		
	elseif data == "vender-cl" then
		TriggerServerEvent("alien-vendinha:weaponfactory","cl")

	elseif data == "vender-cg" then
		TriggerServerEvent("alien-vendinha:weaponfactory","cg")
	...
	while true do
		...
		for k,v in pairs(factory) do
			...
			if distance <= 1.5 and not onmenu then
				idle = 5
				DrawText3D(factory.x,factory.y,factory.z,"Pressione [E] Para Vender Orgânicos.")
			end
		end
		Citizen.Wait(idle)
	end
