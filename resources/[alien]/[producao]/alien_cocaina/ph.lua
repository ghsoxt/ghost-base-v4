...
src = {}
Tunnel.bindInterface("alien_cocaina", src)
...
RegisterServerEvent("alien_cocaina:weaponfactory")
AddEventHandler("alien_cocaina:weaponfactory",function(item)
	local src = source
				...
				if not inProgress[src] then
					if item == "coca" then
						if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("cocaina") <= vRP.getInventoryMaxWeight(user_id) then
							if vRP.getInventoryItemAmount(user_id,"folhadecoca") >= 50 then
								if vRP.getInventoryItemAmount(user_id,"pastadecoca") >= 50 then
											if vRP.tryGetInventoryItem(user_id,"folhadecoca",50) and vRP.tryGetInventoryItem(user_id,"pastadecoca",50) then
										...
										TriggerClientEvent("Notify",src,"negado","Você não tem <b>pastadecoca</b> o suficiente.")
									end
						else
					...
					elseif item == "cocaa" then
						if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("cocaina") <= vRP.getInventoryMaxWeight(user_id) then
							if vRP.getInventoryItemAmount(user_id,"folhadecoca") >= 100 then
								if vRP.getInventoryItemAmount(user_id,"pastadecoca") >= 100 then
											if vRP.tryGetInventoryItem(user_id,"folhadecoca",100) and vRP.tryGetInventoryItem(user_id,"pastadecoca",100) then
												TriggerClientEvent("progress",src,1000,"fazendo")
												vRPclient._playAnim(src,false,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
												inProgress[src] = true
												TriggerClientEvent("F6Cancel",src,true)
...
function src.checkPermission()
	...
	if vRP.hasPermission(user_id,"cocaina.permissao") then
		return true
	end
end