...
dM = {}
Tunnel.bindInterface("alien_arma",dM)
-----------------------------------------------------------------------------------------------------------------------------------------
--[ DM ] WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookarmamento = "---------------------1082550777526096013/IYkZS6prHZnSYOUrsEtkNwZC5Eq_KLqLQu7f21KBNSk4yorPJ3jYlqRm8MlJ_1bz7on-"
...
--[ DM ] EVENTOS 
-----------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("produzir-armamento")
AddEventHandler("produzir-armamento",function(item)
	local source = source
    local user_id = vRP.getUserId(source)
    local identity = vRP.getUserIdentity(user_id)
	if user_id then
		for k,v in pairs(armamento) do
			if item == v.item then
                if item == "ak" then
                    if vRP.hasPermission(user_id,"armas.permissao") or vRP.hasPermission(user_id,"armas.permissao") then
					if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wbody|WEAPON_ASSAULTRIFLE") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"placa-metal") >= 100 then
                            if vRP.getInventoryItemAmount(user_id,"pecadearma") >= 100 then
                                
                                    if vRP.tryGetInventoryItem(user_id,"placa-metal",100) and vRP.tryGetInventoryItem(user_id,"pecadearma",100) then
                                        ...
                                        SendWebhookMessage(webhookarmamento,"```prolog\n[ID]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[PRODUZIU]: 1X DE AK47 "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
                ...
                elseif item == "fiveseven" then
					...
					if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wbody|WEAPON_PISTOL_MK2") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"placa-metal") >= 40 then
                            if vRP.getInventoryItemAmount(user_id,"pecadearma") >= 40 then

                                    if vRP.tryGetInventoryItem(user_id,"placa-metal",40) and vRP.tryGetInventoryItem(user_id,"pecadearma",40) then
                                        TriggerClientEvent("fechar-nui-arma",source)
                ...
                elseif item == "uzi" then
					...
					if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wbody|WEAPON_MICROSMG") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"placa-metal") >= 50 then
                            if vRP.getInventoryItemAmount(user_id,"pecadearma") >= 50 then
            
                                    if vRP.tryGetInventoryItem(user_id,"placa-metal",50) and vRP.tryGetInventoryItem(user_id,"pecadearma",50)  then
                                        TriggerClientEvent("fechar-nui-arma",source)
                ...
                elseif item == "mtar" then
					...
					if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wbody|WEAPON_ASSAULTSMG") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"placa-metal") >= 60 then
                            if vRP.getInventoryItemAmount(user_id,"pecadearma") >= 60 then
                                
                                    if vRP.tryGetInventoryItem(user_id,"placa-metal",60) and vRP.tryGetInventoryItem(user_id,"pecadearma",60)  then
                                        TriggerClientEvent("fechar-nui-arma",source)
					...
					if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wbody|WEAPON_GUSENBERG") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"placa-metal") >= 80 then
                            if vRP.getInventoryItemAmount(user_id,"pecadearma") >= 80 then
                                    ...
                                    if vRP.tryGetInventoryItem(user_id,"placa-metal",80) and vRP.tryGetInventoryItem(user_id,"pecadearma",80) then
                                        TriggerClientEvent("fechar-nui-arma",source)
					...
					if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wammo|WEAPON_ASSAULTRIFLE") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"polvora") >= 50 then
                            if vRP.getInventoryItemAmount(user_id,"capsula") >= 50 then
                                    ...
                                    if vRP.tryGetInventoryItem(user_id,"polvora",50) and vRP.tryGetInventoryItem(user_id,"capsula",50)  then
                                        TriggerClientEvent("fechar-nui-arma",source)
                                ...
                                TriggerClientEvent("Notify",source,"negado","Você precisa de <b>50x Capsula</b>.")
                            end
                        else
					...
					if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wammo|WEAPON_PISTOL_MK2") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"polvora") >= 30 then
                            if vRP.getInventoryItemAmount(user_id,"capsula") >= 20 then
                                    ...
                                    if vRP.tryGetInventoryItem(user_id,"polvora",30) and vRP.tryGetInventoryItem(user_id,"capsula",20)  then
                                        TriggerClientEvent("fechar-nui-arma",source)
                                ...
                                TriggerClientEvent("Notify",source,"negado","Você precisa de <b>30x Capsula</b>.")
                            end
                        else
					...
					if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("wammo|WEAPON_ASSAULTSMG") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"polvora") >= 40 then
                            if vRP.getInventoryItemAmount(user_id,"capsula") >= 50 then
                                    ...
                                    if vRP.tryGetInventoryItem(user_id,"polvora",40) and vRP.tryGetInventoryItem(user_id,"capsula",50) then
                                        TriggerClientEvent("fechar-nui-arma",source)
