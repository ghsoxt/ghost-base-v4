...
src = {}
Tunnel.bindInterface("alien_placa", src)
...
RegisterServerEvent("alien_placa:weaponfactory")
AddEventHandler("alien_placa:weaponfactory",function(item)
	local src = source
				...
				if not inProgress[src] then
					if item == "placa" then
						if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("placa") <= vRP.getInventoryMaxWeight(user_id) then
							if vRP.getInventoryItemAmount(user_id,"aluminio") >= 10 then
								if vRP.getInventoryItemAmount(user_id,"aluminio") >= 10 then
									if vRP.getInventoryItemAmount(user_id,"fio") >= 10 then
										if vRP.getInventoryItemAmount(user_id,"fio") >= 10 then
											if vRP.tryGetInventoryItem(user_id,"aluminio",20) and vRP.tryGetInventoryItem(user_id,"aluminio",30) and vRP.tryGetInventoryItem(user_id,"fio",30) and vRP.tryGetInventoryItem(user_id,"fio",20) then
								...
								if vRP.getInventoryItemAmount(user_id,"plastico") >= 4 then
									if vRP.getInventoryItemAmount(user_id,"aluminio") >= 2 then
										if vRP.getInventoryItemAmount(user_id,"chave") >= 2 then
											if vRP.tryGetInventoryItem(user_id,"aluminio",4) and vRP.tryGetInventoryItem(user_id,"plastico",4) and vRP.tryGetInventoryItem(user_id,"aluminio",2) and vRP.tryGetInventoryItem(user_id,"chave",2) then
												TriggerClientEvent("progress",src,1000,"fazendo")
												vRPclient._playAnim(src,false,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
												inProgress[src] = true
												TriggerClientEvent("F6Cancel",src,true)
					...
					elseif item == "nitro" then
						if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("nitro") <= vRP.getInventoryMaxWeight(user_id) then
							if vRP.getInventoryItemAmount(user_id,"aluminio") >= 2 then
								if vRP.getInventoryItemAmount(user_id,"aluminio") >= 2 then
									if vRP.getInventoryItemAmount(user_id,"fio") >= 2 then
										if vRP.getInventoryItemAmount(user_id,"co2") >= 2 then
											if vRP.tryGetInventoryItem(user_id,"aluminio",2) and vRP.tryGetInventoryItem(user_id,"aluminio",2) and vRP.tryGetInventoryItem(user_id,"fio",2) and vRP.tryGetInventoryItem(user_id,"co2",2) then
												TriggerClientEvent("progress",src,1000,"fazendo")
												vRPclient._playAnim(src,false,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
												inProgress[src] = true
												TriggerClientEvent("F6Cancel",src,true)
						...
						if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("nitro") <= vRP.getInventoryMaxWeight(user_id) then
							if vRP.getInventoryItemAmount(user_id,"aluminio") >= 2 then
								if vRP.getInventoryItemAmount(user_id,"plastico") >= 2 then
									if vRP.getInventoryItemAmount(user_id,"aluminio") >= 2 then
										if vRP.getInventoryItemAmount(user_id,"chave") >= 2 then
											if vRP.tryGetInventoryItem(user_id,"aluminio",2) and vRP.tryGetInventoryItem(user_id,"plastico",2) and vRP.tryGetInventoryItem(user_id,"aluminio",2) and vRP.tryGetInventoryItem(user_id,"chave",2) then
												TriggerClientEvent("progress",src,1000,"fazendo")
												vRPclient._playAnim(src,false,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
												inProgress[src] = true
												TriggerClientEvent("F6Cancel",src,true)
...
function src.checkPermission()
	...
	if vRP.hasPermission(user_id,"desmanche.permissao") or vRP.hasPermission(user_id,"motoclub.permissao") then
		return true
	end
end