...
Resg = {}
ems = {}
Tunnel.bindInterface("alien_hp",Resg)
...
--[ WEBHOOK ]----------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookslaudomedico = "---------------------1082552949714190366/hiJkzGgXXaxYJfp8nNGHF9YxkN8ilkahI3j8Eo92_IiOROBYvSd2iO1nS9WgZdPaKR12"
	...
	if args[1] then
		...
		if vRP.hasPermission(user_id,"paramedico.permissao") then
			if user_id then
				...
				SendWebhookMessage(webhooksresgatechat,"```prolog\n[PASSAPORTE]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[CHAT]: ESCREVEU NO CHAT HOSPITAL \n[ESCRITA]: "..rawCommand:sub(4).." "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
			end
		end
	end
end)
...
AddEventHandler("reanimar:pagamento",function()
	local user_id = vRP.getUserId(source)
	vRP.antiflood(source,"reanimar:pagamento",2)
	if user_id then
		vRP.hackban(source,"REANIMAR PAGAMENTO")
...
RegisterCommand('re',function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	function ems.checkServices()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		local emss = vRP.getUsersByPermission("paramedico.permissao") or vRP.hasPermission(user_id,"HpIlegal.permissao")
		if parseInt(#emss) == 0 then
			return true
		end
	end
end
	if vRP.hasPermission(user_id,"paramedico.permissao") or vRP.hasPermission(user_id,"HpIlegal.permissao") and vRPclient.getHealth(source) > 101 then
	...
	elseif vRP.hasPermission(user_id,"policia.permissao") and vRPclient.getHealth(source) > 101 then
	local nplayer = vRPclient.getNearestPlayer(source,2)
		if ems.checkServices() then
			if nplayer then
				if vRPclient.isInComa(nplayer) then
					local identity_user = vRP.getUserIdentity(user_id)
					local nuser_id = vRP.getUserId(nplayer)
    ...
    if vRP.hasPermission(user_id,"paramedico.permissao") then
        ...
        if nplayer then
			if not vRPclient.isComa(nplayer) then
				...
				TriggerClientEvent("Notify",source,"Sucesso","Tentando tratar o paciente.")
            end
        end
    end
end)
