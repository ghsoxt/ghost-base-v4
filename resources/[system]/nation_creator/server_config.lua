...
if multiCharacter then
    ...
    vRP._prepare("nation_creator/update_user_first_spawn","UPDATE vrp_user_identities SET name = @name, firstname = @name2, age = @age WHERE user_id = @id")
    vRP._prepare("nation_creator/create_characters","INSERT INTO vrp_users(id) VALUES(@id)")
    vRP._prepare("nation_creator/remove_characters","UPDATE vrp_users SET deleted = 1 WHERE id = @id")
    vRP._prepare("nation_creator/get_characters","SELECT * FROM vrp_users WHERE id = @steam and deleted = 0")
    vRP._prepare("nation_creator/get_character","SELECT * FROM vrp_users WHERE id = @steam and deleted = 0 and id = @id")
    vRP._prepare("nation_creator/get_charinfo","SELECT * FROM vrp_users WHERE id = @id")
...
function func.saveChar(name, lastname, age, char, id)
    ...
    if name and lastname and age then
        vRP.execute("nation_creator/update_user_first_spawn",{ id = user_id, name = name, name2 = lastname, age = age })
    end
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- registro de tatto no selecionar personagem
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("registro_tatto_select_personagem")
AddEventHandler("registro_tatto_select_personagem",function()
    local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
        setPlayerTattoos(source, user_id)
...
function func.getCharsInfo()
    ...
    for k,v in ipairs(data) do
        --local teste = vRP.query("nation_creator/get_charinfo",{ id = v.id })
        local bank = vRP.getBank(v.id)
...
function func.playChar(info)
    local source = source
    local steam = getPlayerSteam(source)
    local data = vRP.query("nation_creator/get_character",{ steam = steam, id = info.user_id })
    if #data > 0 then
...
RegisterCommand('resetchar',function(source, args) -- COMANDO DE ADMIN PARA RESETAR PERSONAGEM
    if func.checkPermission({"mindmaster.permissao", "Admin"}, source) then
        if args[1] then 
...
RegisterCommand('spawn',function(source) -- COMANDO DE ADMIN PARA SIMULAR O SPAWN
    if func.checkPermission({"mindmaster.permissao"}, source) or not vRP.getUserId(source) then
        if multiCharacter then
