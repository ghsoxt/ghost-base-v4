...
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local inLocate = { 90.94,-1603.65,31.08 } 
		...
		while true do
			if inService and inTimers > 0 then
				...
				if inTimers <= 0 then
					...
					TriggerEvent("NotifyPush",{text = "Olá! Gostaria de estar comprando um produto.", code = 20, title = "Quero comprar um produto", x = pedLocate[rand][1], y = pedLocate[rand][2], z = pedLocate[rand][3], name = callName[math.random(#callName)].." "..callName2[math.random(#callName2)], rgba = {69,115,41} })
				end
			end
