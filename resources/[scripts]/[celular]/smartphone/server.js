...
if (pB.base === 'Alien_v3') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (c, d) => {
    ...
    if (pB.base === 'Alien_v4') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (d, e) => {
        const f = e - (await pp.getBank(d)),
        g = {};
        g.id = d;
        return g.bank = f, pp.execute('vrp_users/addBank', g);
    };else {
        if (pB.base === 'Alien_v5') {
        ...
        } else pB.base === 'Alien_rework' && (pB.serverSideProp = true, pp.getUserSource = pp.Source, pp.getUserId = pp.Passport, pp.getInventoryItemAmount = pp.InventoryItemAmount, pp.hasPermission = pp.HasPermission, pp.request = pp.Request, pp.getBankMoney = async c => {
...
(pB.base === 'Alien_v4' || pB.base === 'Alien_v5') && (pp.getUserSource = pp.userSource, pp.getUsersByPermission = async c => {
    const d = [];
...
const pN = {
    'Alien_v5': 'checkBroken',
    'Alien_rework': 'CheckDamage'
};
const pO = pN;
...
p6(async function () {
    try {
        while (true) {
            ...
            /* const g = await pz.post('http://localhost/smartphone/v2/server/6056b4aa1cf80010efb4ed1d/authenticate', f, d),
...
async function ra(d, e) {
    const f = {};
    f.paypal = e, await pc('vrp_users').update(f).where('id', d);
}
