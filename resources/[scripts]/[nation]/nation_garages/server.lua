...
RegisterCommand('dv',function(source)
    local user_id = vRP.getUserId(source)
    if vRP.hasPermission(user_id,config.dv_permission) or vRP.hasPermission(user_id,"dv.permissao") then
		...
		if veh and nearestPlate then
		 local plateOwnerId = vRP.getUserByRegistration(nearestPlate) or vRP.getVehiclePlate(nearestPlate) 
			if plateOwnerId then
				...
				vRP.elementGebenInventar(user_id,"dinheirosujo",parseInt(getVehiclePrice(name)*0.10))
...
--     local user_id = vRP.getUserId(source)
--     if vRP.hasPermission(user_id,"mindmaster.permissao") or vRP.hasPermission(user_id,"admin.permissao") then
